...
New-ADOrganizationalUnit -Name 'Servers' -Path "$dn_path" -Description 'Servers OU for new objects' -Verbose
# Create a new AD user for SQL installation with the specified details
New-ADUser -Name 'sqlinstall' -GivenName 'SQL' -Surname 'INSTALL' -UserPrincipalName "sqlinstall@$domain_name" -SamAccountName 'sqlinstall' -AccountPassword (ConvertTo-SecureString "$sql_svc_acct_pswd" -AsPlainText -Force) -Enabled $true -Verbose
...
New-ADUser -Name "$sql_svc_acct_user" -GivenName 'SQL' -Surname 'SERVICE' -UserPrincipalName "$sql_svc_acct_user@$domain_name" -SamAccountName $sql_svc_acct_user -AccountPassword (ConvertTo-SecureString "$sql_svc_acct_pswd" -AsPlainText -Force) -Enabled $true -Verbose
# Stop the transcript
Stop-Transcript
